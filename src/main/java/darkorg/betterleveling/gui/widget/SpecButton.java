...
public class SpecButton extends AbstractSpecButton {
    ...
    public void renderWidget(@Nonnull MatrixStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        drawCenteredString(pMatrixStack, minecraft.fontRenderer, this.value.getTranslation(), this.x + 16, this.y - 10, 16777215);
        if (this.isHovered() || this.isFocused()) {
            this.renderToolTip(pMatrixStack, pMouseX, pMouseY);
        }
        this.renderBg(pMatrixStack, minecraft, pMouseX, pMouseY);
        minecraft.getItemRenderer().renderItemAndEffectIntoGUI(this.representativeItemStack, x + 8, y + 8);
    }
